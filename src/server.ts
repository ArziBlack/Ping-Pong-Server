...
let games: Game[] = [];
const privateGames: Record<string, PrivateGame> = {};
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("createPrivateGame", (settings: GameSettings) => {
    ...
    console.log(
      `Private game created with code: ${inviteCode} (${settings.gameMode} mode, speed: ${settings.ballSpeed}, fps: ${settings.frameRate})`
    );
  });

  // Join private game with invite code
  socket.on("joinPrivateGame", async (inviteCode: string) => {
  ...
  socket.on("movePaddle", (data: MovePaddleData) => {
    games.forEach((game) => {
  ...
  socket.on("endGame", (data: EndGameData) => {
...
function handlePlayerDisconnect(disconnectedSocketId: string): void {
  // Remove the player from waitingPlayers
  waitingPlayers = waitingPlayers.filter(
    (player) => player.playerId !== disconnectedSocketId
  );

  // Remove the player from active games and notify the other player
  games.forEach((game) => {
    const disconnectedPlayer =
      game.players.player1.playerId === disconnectedSocketId ||
